...
- 士気40以下は橙色、士気24以下または敗走中は黄色で表示する。
- 敗走中は先頭へ「敗走」を追加する。
...
敗走判定は士気を主条件にし、結束0だけでは敗走しない。結束崩壊は戦闘力、形状維持、ZOC維持を落とすが、勝敗確定は士気崩壊で読む。

同陣営に戦闘可能な別スライムが残っている場合、敗走スライムは非交戦状態で最低8秒経過し、自軍は士気37以上、敵軍は士気38以上まで回復すると`hold`へ立て直せる。
